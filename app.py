...
st.set_page_config(page_title="Live Broad-Market Breakout Hunter", layout="wide")

st.title("⚡ Live Broad-Market Breakout Hunter (Strict 3-Parameter Engine)")
st.markdown("100% Live Exchange Feed | 1. Weekly RSI (69–80) | 2. Volume Surge | 3. Positive 18W ROC")

@st.cache_data(ttl=3600)
def fetch_broad_market_breakouts():
    try:
        raw_tickers = ns.get_nifty_500_with_ns()
    except Exception:
        # Fallback robust list if library fails to fetch
        raw_tickers = [
    ...
    matched_stocks = []
    progress_bar = st.progress(0, text="Scanning live broad-market exchange feeds...")
    ...
    for idx, t in enumerate(raw_tickers):
        ...
        progress_bar.progress(min((idx + 1) / total, 1.0), text=f"Scanned {idx+1}/{total} broad-market stocks live...")
        
    progress_bar.empty()
    return pd.DataFrame(matched_stocks)

with st.spinner("Connecting to live exchange APIs across the market universe..."):
    df_results = fetch_broad_market_breakouts()
...
if not df_results.empty:
    ...
    st.warning("No live stocks currently match all 3 strict parameters simultaneously in this market cycle.")
