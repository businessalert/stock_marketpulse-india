...
# Massive expanded liquid NSE ticker universe to capture all active breakout stocks
...
def fetch_live_breakouts_v2(tickers):
    matched_stocks = []
    
    progress_bar = st.progress(0, text="Scanning live exchange weekly feeds...")
    total = len(tickers)
    ...
    for idx, t in enumerate(tickers):
        try:
            ...
            if len(hist) >= 30:
                ...
                # Volume expansion check (Current weekly volume > 1.2x of 10-week average)
                vol_sma_10 = volume.rolling(window=10).mean().iloc[-1]
                current_vol = volume.iloc[-1]
                volume_surging = current_vol > (1.2 * vol_sma_10) if vol_sma_10 > 0 else True
                ...
                # EXACT USER RULES: Weekly RSI strictly between 69 and 80, Volume surging, ROC (18) > 0
                is_qualified = (
                    (69.0 <= rsi <= 80.0) and 
                    volume_surging and 
                    (roc > 0.0)
                ...
                if is_qualified:
                    matched_stocks.append({
                        ...
                        'Volume Surge': f"{round(current_vol / vol_sma_10, 2)}x Avg" if vol_sma_10 > 0 else "Active",
                        'MFI (14)': round(mfi, 2),
...
with st.spinner("Connecting to live exchange APIs across the expanded NSE universe..."):
    scanned_df = fetch_live_breakouts_v2(EXPANDED_NSE_TICKERS)
...
if view == "⭐ 1. Positional Master Portfolio (Custom Ranking)":
    ...
    if not scanned_df.empty:
        ...
        st.success(f"Successfully generated portfolio with **{len(top_portfolio)}** live-ranked breakout stocks in the 69–80 RSI zone.")
        st.dataframe(top_portfolio, use_container_width=True)
    else:
        st.warning("No stocks currently match the strict 69–80 RSI band in this batch.")

elif view == "⚡ 2. Breakout Hunter: Weekly RSI (69-80) + Volume Surge":
    st.subheader("⚡ Active Breakout Hunter (Strict Weekly RSI 69–80 Band)")
    st.markdown("""
    ### 🎯 Active Confluence Filters:
    * **Weekly RSI Range**: Strictly **69 to 80** (Momentum sweet spot).
    * **Volume Expansion**: Active volume surge (`> 1.2x` moving average).
    * **Momentum Trend**: Positive 18-week Rate of Change (`ROC > 0`).
    """)
    
    if not scanned_df.empty:
        final_ranked = rank_and_select_top_25(scanned_df)
        st.success(f"Found **{len(scanned_df)}** total qualifying stocks. Displaying top-ranked leaders below:")
        st.dataframe(final_ranked, use_container_width=True)
    else:
        st.warning("No live stocks matching the exact RSI 69–80 criteria right now.")
